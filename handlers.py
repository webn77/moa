...
async def on_dm(s, e):
    ...
    if await onboard_catch(s, e, q):          # 처음 오신 분의 한 걸음 — 1번(목표)만 여기서 받는다
        return
    # **물어본 글 아래 스레드로 답한다** (2026-09-22 사장님 지적: 「질문에 대한 대답은 다
    # 스레드였던 거 같아서」). `agent_view` 를 켜면서 「…하는 중」 은 그 글 아래에 붙였는데
    # **답은 원래대로 맨 위로 갔다** — 둘을 같이 옮겼어야 했다. 묻고 답한 짝이 흩어졌다.
    # 정리·순서는 **팀에 올리는 것**이라 프로젝트 방으로 가는 게 맞다 — 그건 그대로 둔다.
