...
async def on_dm(s, e):
    ...
    #
    # 「질문에 대한 답은 스레드로」 로 고쳤더니 **답이 접혀서 안 보였다** — 사장님이
    # 「글 남겨도 작동을 안 하는데?」 라고 하셨고, 봇은 답을 하고 있었다. 접힌 스레드 안이었다.
    # **DM 은 그 자체가 한 대화다.** 스레드를 더하면 층이 하나 늘고 답이 숨는다.
    # 이미 스레드 안에서 물었으면 그 스레드에 답한다 — 그건 사람이 만든 덩이다.
    # 채널은 다르다: 거기서는 여러 이야기가 섞이므로 스레드가 맞다 (카드 스레드가 그렇다).
    th = e.get("thread_ts")
    async with thinking(s, e, say("thinking"), say("thinking_read"), say("thinking_almost")):
        if "현황" in q:
