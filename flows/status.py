...
def plan():
    ...
    **오늘 날짜는 여기서 넘긴다.** `core` 는 `datetime` 을 함수 안에서 부르는데, 골든의
    ...
    snap = lambda: {k: (c.get("start"), c.get("start_src")) for k, c in STATE["cards"].items()}
    before = snap()
    core.plan(STATE["cards"],
              {u: (hours.get(t.get("name")) or 5) / 5 for u, t in team.items()},
              today=datetime.date.today(), hours_fn=need_hours)
    # **바뀌었으면 적어 둔다.** 앱 홈을 열거나 「내 할 일」 을 물을 때도 여기를 지나는데
    # 그 길에는 `redraw` 가 없어서 저장이 안 됐다 — 사람이 옮긴 날이 다시 켜면 사라진다.
    # 안 바뀌었으면 안 쓴다: 홈을 열 때마다 파일을 쓰면 켜자마자 71번 쓴다
    if snap() != before:
        save()
